...
if not exist "C:\Windows\Temp\VBoxGuestAdditions.iso" (
  powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; (New-Object System.Net.WebClient).DownloadFile('http://download.virtualbox.org/virtualbox/7.0.12/VBoxGuestAdditions_7.0.12.iso', 'C:\Windows\Temp\VBoxGuestAdditions.iso')" <NUL
)
